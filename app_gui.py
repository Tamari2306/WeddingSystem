...
<<<<<<< HEAD
        # --- MODIFIED COLUMNS ---
        tree = ttk.Treeview(top, columns=("visual_id", "name", "qr_code_id", "phone", "has_entered", "db_id"), show="headings")
        tree.heading("visual_id", text="Visual ID")
        tree.heading("name", text="Name")
        tree.heading("qr_code_id", text="QR Code ID (Manual Key)") # Highlight this column
        tree.heading("phone", text="Phone")
        tree.heading("has_entered", text="Entered?")
        tree.heading("db_id", text="DB ID") # Hidden but useful for deletion
        # --- END MODIFIED COLUMNS ---

        tree.column("visual_id", width=50)
        tree.column("qr_code_id", width=150) # Make QR ID wider for visibility
        tree.column("db_id", width=0, stretch=tk.NO) # Hide the DB ID column
        ...
        for index, guest in enumerate(guests, start=1):
            ...
            tree.insert("", tk.END, values=(
                index,
                guest.name,
                guest.qr_code_id,  # Moved up for easy reference
                guest.phone,
                "Yes" if guest.has_entered else "No",
                guest.id  # Pass the DB ID for deletion reference
            ))
            # --- END MODIFIED VALUES ---
=======
...
>>>>>>> 3bf9a2313bb922015464406727ad4a2fe3a1e571

        tree.pack(expand=True, fill="both")

        # Add Delete button
<<<<<<< HEAD
        delete_button = ttk.Button(top, text="Delete Guest", command=lambda: delete_selected_guest_from_list(tree, top))
=======
...
<<<<<<< HEAD
# You must also update the delete_selected_guest function to use the new column index:
def delete_selected_guest_from_list(tree, top):
    # ... (inside the function)
    ...
    # guest_id is now at index 5 in the displayed values (the hidden "db_id" column)
    guest_id = tree.item(selected_item, 'values')[5]
    # ... (rest of the logic)
    # Note: You'll also need to update how you call delete_selected_guest to pass the tree and top window.
    # The existing delete_selected_guest function uses 'values[1]', so you might need to adjust column indices carefully.
